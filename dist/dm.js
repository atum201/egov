...
var DataManager = Backbone.Model.extend({
    ...
    initialize: function initialize() {
        ...
        setTimeout(function () {
            if (typeof cachedoc == "undefined") console.log("undefined");else console.log("cachedoc");
        }, 5000);
        if (typeof cachedoc == "undefined") console.log("undefined");else console.log("cachedoc");
    },
    getuserinfo: function getuserinfo(username, callback) {
        var _this = this;
        userdoc.find({ username: username }).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
    ...
    updateusercontact: function updateusercontact(usercontact) {
        var _this = this;
        userdoc.find({ username: usercontact.username }).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
    ...
    getmsgoffline: function getmsgoffline(data, ncontact, callback) {
        var _this = this;
        if (ncontact.length > 0) {
            messagedoc.find({ receiver: data, successful: "0", sender: { $nin: ncontact } }).toArray(function (err, docs) {
                if (!err) {
                    var rc = docs.length;
                    if (rc > 0) {
            ...
            messagedoc.find({ receiver: data, successful: "0" }).toArray(function (err, docs) {
                if (!err) {
                    var rc = docs.length;
                    if (rc > 0) {
                        if (typeof callback == "function") callback(docs);
    ...
    updatecontact: function updatecontact(username, contact, callback) {
        var _this = this;
                ...
                try {
                    if (rc > 0) {
                        ...
                        for (var j = 0; j < contactupdate.length; j++) {
                            ...
                            if (!exit) contacts.push(contactupdate[j]);
                        }
    ...
    updatecontact2: function updatecontact2(id, contacts) {
        var _this = this;
    ...
    removecontact: function removecontact(username, contact) {
        var _this = this;
                ...
                try {
                    if (rc > 0) {
                        ...
                        for (var j = 0; j < contacts.length; j++) {
                            ...
                            if (contacts[j].username === contact.username) index = j;
                        }
    ...
    updatefieldcontact: function updatefieldcontact(username, usercontact, field, value) {
        var _this = this;
        var contactupdate = {};
        try {
            if (field === "timeclean") {
    ...
    updatefieldscontact: function updatefieldscontact(username, usercontact, fields, unset) {
        var _this = this;
    ...
    addmessage: function addmessage(message, callback) {
        var _this = this;
        messagedoc.insert(message, { continueOnError: true }, function (err, docs) {
            if (typeof callback == "function") callback(docs.ops[0]);
        });
    },
    getlastmsg: function getlastmsg(user1, user2, numbermsg, callback) {
        var _this = this;

        if (numbermsg === null) {
            numbermsg = 10;
        }
        userdoc.find({ username: user1, "contact.username": user2 }, { 'contact.$': 1 }).toArray(function (err, docs) {
            var isclose = false;
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
    ...
    updatemessages: function updatemessages(data) {
        var _this = this;
        //console.log(data);
    ...
    getmessagehitory: function getmessagehitory(data, callback) {
        var _this = this;

        userdoc.find({ username: data.sender, "contact.username": data.receiver }, { 'contact.$': 1 }).toArray(function (err, docs) {
            var isclose = false;
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
            ...
            } else console.log("getmessagehitory3");
        });
    },
    getmsgbyid: function getmsgbyid(idmsg, callback) {
        var _this = this;
        messagedoc.find({ _id: ObjectID(idmsg) }).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
                    callback(docs[0]);
    ...
    setlastconnect: function setlastconnect(data) {
        var _this = this;
        var d = new Date();
    ...
    addusertocache: function addusertocache(data) {
        var _this = this;
        _this.updateobject("cache", { cacheid: 1 }, { $addToSet: { useronline: data } });
    },
    setuseronline: function setuseronline(data, online) {
        // set online offline user in all contacted.
        var _this = this;
    ...
    setuseroffline: function setuseroffline(data) {
        var _this = this;
        _this.setuseronline(data, false);
    },
    setforreset: function setforreset() {
    ...
    getloginstore: function getloginstore(data, callback) {
        var _this = this;
        ...
        var datareturn = 0;
        var cm = new Common();
        userdoc.find({ username: data.username }).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
                    var loginstore = docs[0].loginstore;
                        ...
                        if (month === "all" || month === '' || month === 'undefined') {
                            if (firstconn.getFullYear <= year) {
                                ...
                                for (var j = index; j > 0; j--) {
                                    var k = 0;
                                    if (k < 12 && typeof store[j] !== 'undefined') datareturn += store[j];
                                    k++;
                                }
                            }
                        } else {
                            var index = cm.monthdiff(firstconn, new Date(year, month, 1));
                            if (typeof store[index] !== 'undefined') datareturn = store[index];
                        }
                    }
                }
            }
    ...
    getloginstores: function getloginstores(data, callback) {
        var _this = this;

        var month = data.month;
        if (typeof data.month === 'undefined') month = "all";
        var year = data.year;
        if (typeof data.year === 'undefined') year = 'all';
        var datareturn = [];
        var cm = new Common();
        userdoc.find({ username: { $in: data.users } }).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
                    try {
                                ...
                                if (month === "all" || month === '' || month === 'undefined') {
                                    if (firstconn.getFullYear() <= year) {
                                        ...
                                        for (var j = index; j > 0; j--) {
                                            //console.log(j+'-----');
                                            var k = 0;
                                            if (k < 12 && typeof store[j] !== 'undefined') userstore.login += store[j];
                                            k++;
                                        }
                                    } else {
                                    ...
                                    if (typeof store[index] !== 'undefined') userstore.login = store[index];
                                }
                            }
                            datareturn.push(userstore);
                        }
                    } catch (eere) {
                        console.log("getloginstores2" + eere);
                    }
                }
    ...
    // database core
    createcollection: function createcollection(colname) {},
    createcollectionoption: function createcollectionoption(colname, option) {},

    updateobject: function updateobject(col, query, update) {
        var _this = this;
    ...
    updateobjectoption: function updateobjectoption(col, query, update, option) {
        var _this = this;
        var collection = db.collection(col);
    ...
    updateobjectcallback: function updateobjectcallback(col, query, update, callback) {
        var _this = this;
        var collection = db.collection(col);
        collection.update(query, update, function (err) {
            if (err) {
    ...
    updateobjectoptioncallback: function updateobjectoptioncallback(col, query, update, option, callback) {
        var _this = this;
        var collection = db.collection(col);
        collection.update(query, update, option, function (err) {
            if (err) {
    ...
    insertobject: function insertobject(col, object) {
        var _this = this;
        var collection = db.collection(col);
    ...
    insertobjectoption: function insertobjectoption(col, object, option) {
        var _this = this;
        var collection = db.collection(col);
    ...
    insertobjectcallback: function insertobjectcallback(col, object, callback) {
        var _this = this;
        var collection = db.collection(col);
        collection.insert(object, function (err, docs) {
            if (!err) {
    ...
    insertobjectoptioncallback: function insertobjectoptioncallback(col, object, option, callback) {
        var _this = this;
        var collection = db.collection(col);
        collection.insert(object, option, function (err, docs) {
            if (!err) {
    ...
    removeobject: function removeobject(col, query) {
        var _this = this;
        var collection = db.collection(col);
    ...
    removeobjectoption: function removeobjectoption(col, query, option) {
        var _this = this;
        var collection = db.collection(col);
    ...
    removeobjectcallback: function removeobjectcallback(col, query, callback) {
        var _this = this;
        var collection = db.collection(col);
    ...
    removeobjectoptioncallback: function removeobjectoptioncallback(col, query, option, callback) {
        var _this = this;
        var collection = db.collection(col);
    ...
    findobjectcallback: function findobjectcallback(col, query, callback) {
        var _this = this;
        var collection = db.collection(col);
    ...
    findobjectoptioncallback: function findobjectoptioncallback(col, query, option, callback) {
        var _this = this;
        var collection = db.collection(col);
    ...
    findobjectlimitcallback: function findobjectlimitcallback(col, query, select, limit, callback) {
        var _this = this;
        var collection = db.collection(col);
        collection.find(query, select).limit(limit).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
                    if (typeof callback == "function") callback(docs);
    ...
    findobjectsortlimitcallback: function findobjectsortlimitcallback(col, query, sort, limit, callback) {
        var _this = this;
        var collection = db.collection(col);
        collection.find(query).sort(sort).limit(limit).toArray(function (err, docs) {
            if (!err) {
                var rc = docs.length;
                if (rc > 0) {
                    if (typeof callback == "function") {
